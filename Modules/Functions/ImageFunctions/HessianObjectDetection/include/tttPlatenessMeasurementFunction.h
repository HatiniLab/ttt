...
template<typename TFloat> class PlatenessMeasurementFunction:public ObjectnessMeasurementFunction
{
public:
  ...
  virtual double ComputeObjectProperty(const FixedArray< TFloat,3> & eigenValue) {
		//
		...
		if (Lambda3 >= 0.0 || vnl_math_abs(Lambda3) < std::numeric_limits<double>::epsilon()) {
			return 0;
		} else {
			return sqrt(vnl_math_sqr(Lambda1) + vnl_math_sqr(Lambda2) + vnl_math_sqr(Lambda3));
#if 0
