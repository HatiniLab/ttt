...
TEST(PlatenessTest, Regular) {
  ...
  // Initialize the content of Image A
   ...
   typedef ttt::PlatenessMeasurementFunction<double> PlatenessFunctionType;
   typedef ttt::MultiScaleHessianSmoothed3DToObjectnessMeasureImageFilter<ttt::PlatenessMeasurementFunction<double>,myImageType> PlatenessFilterType;

   PlatenessFunctionType::Pointer platenessFunction = PlatenessFunctionType::New();

   PlatenessFilterType::Pointer filter = PlatenessFilterType::New();

   filter->SetInput(inputImage);
   filter->SetSigmaMin(0.0001);
   filter->SetSigmaMax(0.5);
   filter->SetNumberOfSigmaSteps(5);
   filter->SetObjectnessMeasurementFunction(platenessFunction);
   filter->Update();
