...
AJEnhancementDiffusionImageFilter<TInputImage, TOutputImage>
::AJEnhancementDiffusionImageFilter()
{
  ...
  //set the function
  typename AnisotropicDiffusionPlateEnhancementFunction<UpdateBufferType>::Pointer q
      = AnisotropicDiffusionPlateEnhancementFunction<UpdateBufferType>::New();
 ...
 ::InitializeIteration()
{
  ...
  AnisotropicDiffusionPlateEnhancementFunction<UpdateBufferType> *f = 
     dynamic_cast<AnisotropicDiffusionPlateEnhancementFunction<UpdateBufferType> *>
  ...
  if ( m_TimeStep > ratio ) 
    ...
    itkWarningMacro(<< std::endl << "Anisotropic diffusion unstable time step:" 
                    ...
                    << "for this image is " 
                    << ratio );
    itkWarningMacro(<< std::endl << "Readjusted to "<< ratio);
    m_TimeStep=ratio;
    }
...
::UpdateDiffusionTensorImage()
{
  ...
  while( !it.IsAtEnd() )
    ...
    double plateNessValue = static_cast<double> (im.Get());
    
    Lambda1 = Lambda2 = 1 + ( m_WStrength - 1 ) * vcl_pow ( plateNessValue, iS ); 
    Lambda3 = 1 + ( m_Epsilon - 1 ) * vcl_pow ( plateNessValue, iS ); 
...
template <class TInputImage, class TOutputImage>
typename
AJEnhancementDiffusionImageFilter<TInputImage, TOutputImage>::TimeStepType
AJEnhancementDiffusionImageFilter<TInputImage, TOutputImage>
::CalculateChange()
{
