...
namespace ttt
{
template< typename TLevelSet,typename TVoronoiImage, typename TInputMesh,typename TSpeedImage >
FastMarchingImageFilterWithMesh< TLevelSet,TVoronoiImage,TInputMesh, TSpeedImage >
  ...
  m_TrialHeap()
{
  ...
  m_ClusterImage = ClusterImageType::New();
...
::PrintSelf(std::ostream & os, itk::Indent indent) const
{
  ...
  os << indent << "Large Value: "
     << static_cast< typename itk::NumericTraits< PixelType >::PrintType >( m_LargeValue )
     << std::endl;
...
::Initialize(LevelSetImageType *output)
{
  ...
  m_ClusterImage->CopyInformation(output);
  m_ClusterImage->SetBufferedRegion(output->GetBufferedRegion());
  m_ClusterImage->Allocate();
  m_ClusterImage->FillBuffer(-1);

  // set all output value to infinity
  typedef itk::ImageRegionIterator< LevelSetImageType >
  OutputIterator;
  ...
  if ( m_TrialPoints )
    ...
    while( pointsIter != pointsEnd )
      ...
      if ( m_BufferedRegion.IsInside( idx ) )
        ...
        m_ClusterImage->SetPixel(idx,cluster++);
...
::GenerateData()
{
	m_PointsLocator = PointsLocatorType::New();
	m_PointsLocator->SetPoints(this->GetInputMesh()->GetPoints());
  ...
  LevelSetImageType *output)
{
  ...
  int cluster = m_ClusterImage->GetPixel(index);
  ...
  for ( unsigned int j = 0; j < SetDimension; j++ )
    ...
    if(this->GetVoronoiImage()->GetPixel(neighIndex)==cluster){

    	typename SpeedImageType::PointType neighPoint;
    	const typename SpeedImageType::Pointer speedImage=this->GetInput();
    	speedImage->TransformIndexToPhysicalPoint(neighIndex,neighPoint);

    	auto projectionIndex = m_PointsLocator->FindClosestPoint(neighPoint);

    	auto projectedPoint = this->GetInputMesh()->GetPoint(projectionIndex);

    	this->GetInput()->TransformPhysicalPointToIndex(projectedPoint,neighIndex);
...
::UpdateValue(
  const IndexType & index,
  const SpeedImageType *speedImage,
  LevelSetImageType *output, int cluster)
{
  IndexType neighIndex = index;
  ...
  if ( solution < m_LargeValue )
    ...
    m_ClusterImage->SetPixel(index,cluster);
