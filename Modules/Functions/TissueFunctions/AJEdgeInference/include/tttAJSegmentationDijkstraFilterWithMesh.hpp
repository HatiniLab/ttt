...
ttt::AJSegmentationDijkstraFilterWithMesh<TInputAJGraph, TPlatenessImage, TVertexnessImage, TInputMesh,TOutputAJGraph>::AJSegmentationDijkstraFilterWithMesh() {
...
template<class TInputAJGraph, class TPlatenessImage, class TVertexnessImage, class TInputMesh,class TOutputAJGraph>
void ttt::AJSegmentationDijkstraFilterWithMesh<TInputAJGraph, TPlatenessImage, TVertexnessImage, TInputMesh,TOutputAJGraph>::SetInputAJGraph(
		const TInputAJGraph * ajgraph) {
	this->SetNthInput(0, const_cast<TInputAJGraph*>(ajgraph));
}

template<class TInputAJGraph, class TPlatenessImage, class TVertexnessImage, class TInputMesh,class TOutputAJGraph>
void ttt::AJSegmentationDijkstraFilterWithMesh<TInputAJGraph, TPlatenessImage, TVertexnessImage, TInputMesh,TOutputAJGraph>::SetPlatenessImage(
		const TPlatenessImage * image) {
	this->SetNthInput(1, const_cast<TPlatenessImage*>(image));
}

template<class TInputAJGraph, class TPlatenessImage, class TVertexnessImage, class TInputMesh,class TOutputAJGraph>
void ttt::AJSegmentationDijkstraFilterWithMesh<TInputAJGraph, TPlatenessImage, TVertexnessImage, TInputMesh,TOutputAJGraph>::SetVertexnessImage(
		const TVertexnessImage * image) {
	this->SetNthInput(2, const_cast<TVertexnessImage*>(image));
}
template<class TInputAJGraph, class TPlatenessImage, class TVertexnessImage, class TInputMesh,class TOutputAJGraph>
void ttt::AJSegmentationDijkstraFilterWithMesh<TInputAJGraph, TPlatenessImage, TVertexnessImage, TInputMesh,TOutputAJGraph>::SetInputMesh(
		const TInputMesh * mesh) {
	this->SetNthInput(3, const_cast<TInputMesh*>(mesh));
}
...
template<class TInputAJGraph, class TPlatenessImage, class TVertexnessImage, class TInputMesh,class TOutputAJGraph>
void ttt::AJSegmentationDijkstraFilterWithMesh<TInputAJGraph, TPlatenessImage, TVertexnessImage, TInputMesh,TOutputAJGraph>::ComputeSpeedImage() {
...
template<class TInputAJGraph, class TPlatenessImage, class TVertexnessImage, class TInputMesh,class TOutputAJGraph>
void ttt::AJSegmentationDijkstraFilterWithMesh<TInputAJGraph, TPlatenessImage, TVertexnessImage, TInputMesh,TOutputAJGraph>::InitDefGraph() {
...
template<class TInputAJGraph, class TPlatenessImage, class TVertexnessImage, class TInputMesh,class TOutputAJGraph>
void ttt::AJSegmentationDijkstraFilterWithMesh<TInputAJGraph, TPlatenessImage, TVertexnessImage, TInputMesh,TOutputAJGraph>::GetNeighbors(const IndexType & index,
...
template<class TInputAJGraph, class TPlatenessImage, class TVertexnessImage, class TInputMesh,class TOutputAJGraph>
void ttt::AJSegmentationDijkstraFilterWithMesh<TInputAJGraph, TPlatenessImage, TVertexnessImage, TInputMesh,TOutputAJGraph>::DoFastMarching() {
	...
	//fastMarching->SetInputMesh(mesh);
...
template<class TInputAJGraph, class TPlatenessImage, class TVertexnessImage, class TInputMesh,class TOutputAJGraph>
void ttt::AJSegmentationDijkstraFilterWithMesh<TInputAJGraph, TPlatenessImage, TVertexnessImage, TInputMesh,TOutputAJGraph>::PruneLevelSet(double value) {
	...
	typedef itk::ImageFileWriter<LevelSetImageType> LabelWriterType;

	typename LabelWriterType::Pointer labelWriter = LabelWriterType::New();

	labelWriter->SetFileName("BeforeLevelSet.mha");
	labelWriter->SetInput(m_LevelSet);
	labelWriter->Update();
	}

	typename LevelSetImageType::Pointer levelSetBis = LevelSetImageType::New();

	levelSetBis->SetRegions(m_LevelSet->GetLargestPossibleRegion());
	levelSetBis->CopyInformation(m_LevelSet);
	levelSetBis->Allocate();
	levelSetBis->FillBuffer(-1);

	typename LabelImageType::Pointer labelBis = LabelImageType::New();
	labelBis->SetRegions(m_LevelSet->GetLargestPossibleRegion());
	labelBis->CopyInformation(m_LevelSet);
	labelBis->Allocate();
	labelBis->FillBuffer(-1);

	for(auto pointIt = this->GetMesh()->GetPoints()->Begin();pointIt!=this->GetMesh()->GetPoints()->End();++pointIt){
		auto point = pointIt->Value();

		typename PlatenessImageType::IndexType index;
		this->GetPlatenessImage()->TransformPhysicalPointToIndex(point,index);
		auto value = m_LevelSet->GetPixel(index);
		levelSetBis->SetPixel(index,value);

		auto label = m_Labels->GetPixel(index);
		labelBis->SetPixel(index,label);
	}
	m_LevelSet=levelSetBis;
	m_Labels=labelBis;
	...
	labelWriter->SetFileName("AfterLevelSet.mha");
	labelWriter->SetInput(levelSetBis);
	labelWriter->Update();
	}
}
#if 0
...
#endif
template<class TInputAJGraph, class TPlatenessImage, class TVertexnessImage, class TInputMesh,class TOutputAJGraph>
void ttt::AJSegmentationDijkstraFilterWithMesh<TInputAJGraph, TPlatenessImage, TVertexnessImage, TInputMesh,TOutputAJGraph>::BuildGraph() {
#if 0
...
template<class TInputAJGraph, class TPlatenessImage, class TVertexnessImage, class TInputMesh,class TOutputAJGraph>
void ttt::AJSegmentationDijkstraFilterWithMesh<TInputAJGraph, TPlatenessImage, TVertexnessImage, TInputMesh,TOutputAJGraph>::StoreLabels() {
#if 0
	itk::ImageRegionIterator<LabelImageType> labelIterator(m_Labels,m_Labels->GetLargestPossibleRegion());
...
template<class TInputAJGraph, class TPlatenessImage, class TVertexnessImage, class TInputMesh,class TOutputAJGraph>
void ttt::AJSegmentationDijkstraFilterWithMesh<TInputAJGraph, TPlatenessImage, TVertexnessImage, TInputMesh,TOutputAJGraph>::GenerateData() {
	this->ComputeSpeedImage();
	this->InitDefGraph();
	this->DoFastMarching();

	//this->ComputeLevelsetThreshold(0.3);
	m_LevelSetThreshold=100;
