 ...
 */
template<typename TLevelSet, typename TVoronoiImage, typename TInputMesh, typename TSpeedImage = itk::Image<float, TLevelSet::ImageDimension> >
class FastMarchingImageFilterWithMesh: public itk::ImageToImageFilter<TSpeedImage, TLevelSet> {
public:
	/** Standard class typdedefs. */
	typedef FastMarchingImageFilterWithMesh Self;
	...
	itkStaticConstMacro(SetDimension, unsigned int,
			LevelSetType::SetDimension);itkStaticConstMacro(SpeedImageDimension, unsigned int,
			SpeedImageType::ImageDimension);

	/** Index typedef support. */
	typedef itk::Index<itkGetStaticConstMacro(SetDimension)> IndexType;
	...
	typedef itk::Image<unsigned char, itkGetStaticConstMacro(SetDimension)> LabelImageType;

	/** LabelImagePointer typedef support. */
	typedef typename LabelImageType::Pointer LabelImagePointer;

	typedef itk::Image<int, itkGetStaticConstMacro(SetDimension)> ClusterImageType;

	typedef typename ClusterImageType::Pointer ClusterImagePointer;

	template<typename TPixel>
	void SetBinaryMask(itk::Image<TPixel, SetDimension>* iImage) {
		typedef itk::Image<TPixel, SetDimension> InternalImageType;
		typedef itk::ImageRegionConstIteratorWithIndex<InternalImageType> InternalRegionIterator;
		InternalRegionIterator b_it(iImage, iImage->GetLargestPossibleRegion());
		b_it.GoToBegin();

		TPixel zero_value = itk::NumericTraits<TPixel>::Zero;
	...
	void SetVoronoiImage(const VoronoiImageType * voronoiImage) {
		this->SetNthInput(1, const_cast<VoronoiImageType*>(voronoiImage));
	}

	void SetInputMesh(const InputMeshType * mesh) {
		this->SetNthInput(2, const_cast<InputMeshType*>(mesh));
	}

	const VoronoiImageType * GetVoronoiImage() {
		return dynamic_cast<VoronoiImageType*>(this->itk::ProcessObject::GetInput(1));
	}
	const InputMeshType * GetInputMesh() {
		return dynamic_cast<InputMeshType*>(this->itk::ProcessObject::GetInput(2));
	}
	...
	itkConceptMacro( SameDimensionCheck,
			( itk::Concept::SameDimension< SetDimension, SpeedImageDimension > ) );
		itkConceptMacro( SpeedConvertibleToDoubleCheck,
				( itk::Concept::Convertible< typename TSpeedImage::PixelType, double > ) );
		itkConceptMacro( DoubleConvertibleToLevelSetCheck,
				( itk::Concept::Convertible< double, PixelType > ) );
		itkConceptMacro( LevelSetOStreamWritableCheck,
				( itk::Concept::OStreamWritable< PixelType > ) );
		// End concept checking
#endif

	protected:
		FastMarchingImageFilterWithMesh();
		~FastMarchingImageFilterWithMesh() {}
		...
		virtual double UpdateValue(const IndexType & index,
				const SpeedImageType *, LevelSetImageType *,int cluster);
	...
	private:
		FastMarchingImageFilterWithMesh(const Self &); //purposely not implemented
		...
		typedef itk::PointsLocator<typename TInputMesh::PointsContainer> PointsLocatorType;
...
#include "itkMyFastMarchingImageFilterWithMesh.hxx"
#endif
